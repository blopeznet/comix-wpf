...
namespace DirectoryBrowser.Host.Utils
{
    /// <summary>
    /// Class for generate into system exception for URL param
    /// </summary>
    public static class AclHelper
    {

        public static void AddANewddress(string address = "http://+:8000/Service/")
        {
            if (!AclHelper.FindUrlPrefix(address))
            {
                string args = string.Format(@"http add urlacl url={0} user=Todos", address);
